...
-(NSString*)ArrangeDeviceInfo {
    ...
    [contextDic setValue:[self GetUDId] forKey:@"idfa"];
...
{
    NSUserDefaults *installInfo = [NSUserDefaults standardUserDefaults];
    [installInfo setObject:appKey forKey:@"InstallappKey"];
...
-(void)SendBODYMessage:(CommandFunc)COMDFunc {
    ...
    _groupId=[[DataBase sharedMyDataBase] getGroupID];
    _groupId++;
    //    NSLog(@"groupID:%d",_groupId);
    [[DataBase sharedMyDataBase] insertData:body groupID:_groupId];
...
-(void)sendURLConnection:(NSString*) newUrl{
    //    [self sendURLConnectionAgain];
    NSString *url = newUrl;
    NSArray *array=[[DataBase sharedMyDataBase] fillData];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response  {
    if (arrayCount>0) {
        for (int i = 0; i<arrayCount; i++) {
            ...
            NSInteger changeID = [[DataBase sharedMyDataBase] getID];
            [[DataBase sharedMyDataBase] changeSaveMark:0 groupid:changeID];
            [[DataBase sharedMyDataBase] removeData];
        }
        arrayCount = 0;
    }
    else{
        ...
        NSInteger changeId=[[DataBase sharedMyDataBase] getGroupID];
        [[DataBase sharedMyDataBase] changeSaveMark:0 groupid:changeId];
        [[DataBase sharedMyDataBase] removeData];
