...
NSString *const RYkSSKeychainErrorDomain = @"com.samsoffes.sskeychain";

NSString *const RYkSSKeychainAccountKey = @"acct";
NSString *const RYkSSKeychainCreatedAtKey = @"cdat";
NSString *const RYkSSKeychainClassKey = @"labl";
NSString *const RYkSSKeychainDescriptionKey = @"desc";
NSString *const RYkSSKeychainLabelKey = @"labl";
NSString *const RYkSSKeychainLastModifiedKey = @"mdat";
NSString *const RYkSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef RYSSKeychainAccessibilityType = NULL;
#endif

@interface RYSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation RYSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:RYkSSKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (RYSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return RYSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (RYSSKeychainAccessibilityType) {
		CFRelease(RYSSKeychainAccessibilityType);
	}
	RYSSKeychainAccessibilityType = accessibilityType;
}
#endif
