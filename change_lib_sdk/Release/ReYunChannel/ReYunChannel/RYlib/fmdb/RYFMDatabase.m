...
@interface RYFMDatabase ()

- (RYFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
@end

@implementation RYFMDatabase
...
- (void)clearCachedStatements {
    ...
    for (RYFMStatement *cachedStmt in [_cachedStatements objectEnumerator]) {
        [cachedStmt close];
    }
...
- (void)closeOpenResultSets {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
        RYFMResultSet *rs = (RYFMResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(RYFMResultSet *)resultSet {
    NSValue *setValue = [NSValue valueWithNonretainedObject:resultSet];
    
    [_openResultSets removeObject:setValue];
}

- (RYFMStatement*)cachedStatementForQuery:(NSString*)query {
    return [_cachedStatements objectForKey:query];
}

- (void)setCachedStatement:(RYFMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    RYFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (NSError*)lastError {
    return [NSError errorWithDomain:@"RYFMDatabase" code:sqlite3_errcode(_db) userInfo:[NSDictionary dictionaryWithObject:[self lastErrorMessage] forKey:NSLocalizedDescriptionKey]];
}
...
- (RYFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
- (RYFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    RYFMStatement *statement  = 0x00;
    RYFMResultSet *rs         = 0x00;
    ...
    if (!statement) {
        statement = [[RYFMStatement alloc] init];
    ...
    rs = [RYFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (RYFMResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (RYFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (RYFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[RYFMStatement alloc] init];
...
void ChannelFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv);
void ChannelFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &ChannelFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &ChannelFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
