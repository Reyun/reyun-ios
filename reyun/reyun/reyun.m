...
//#define checkversionurl @"http://192.168.1.141:8642/receive/receive"         //暂时新型服务器

//#define checkversionurl @"http://log.reyun.com/receive/receive"         //热云网服务器新
//#define checkversionurlServerTime @"http://log.reyun.com/receive/gettime" //获得热云外网服务器时间
//#define checkversionurl @"http://log.tai-win.com.tw/receive/receive" //台湾用
//#define checkversionurlServerTime @"http://log.tai-win.com.tw/receive/gettime" //台湾用获得系统时间

//#define checkversionurl @"http://log.tai-win.com.tw"         //热云网服务器新
...
+ (void)initWithAppKey:(NSString *)appKey channelID:(NSString*)channelID{
    NSUserDefaults *installInfo = [NSUserDefaults standardUserDefaults];
    [installInfo setObject:appKey forKey:@"InstallappKeys"];
    [installInfo setObject:channelID forKey:@"channelId"];
...
-(NSString*)GetUDId {
    ...
    {
        NSLog(@"大于6级");
        idfa = [SSKeychain passwordForService:@"serviceID" account:@"SSToolkitTestAccount"];
    }
    else
    {
        NSLog(@"小于6级");
    ...
    if ([idfa length]==0) {
        ...
        [SSKeychain setPassword:writeIDFAToKeychain forService:@"serviceID" account:@"SSToolkitTestAccount"];
        idfa = [SSKeychain passwordForService:@"serviceID" account:@"SSToolkitTestAccount"];
    }
    return idfa;
...
-(void)SendBODYMessage:(CommandFunc)COMDFunc {
    ...
    [[MyDataBase sharedMyDataBase] insertData:body groupID:_groupId];
...
-(void)sendURLConnection:(NSString*) newUrl{
    //    [self sendURLConnectionAgain];
    NSString *url = newUrl;
    NSArray *array=[[MyDataBase sharedMyDataBase] fillData];
...
-(void)FirstGetServersTime{
    //    NSURL *nsurl=[[NSURL alloc]initWithString:checkversionurlServerTime];
    //    NSURLRequest *request=[[NSURLRequest alloc]initWithURL:nsurl cachePolicy:NSURLRequestReloadIgnoringCacheData timeoutInterval:15];
    //    [nsurl release];
    //    NSMutableData *data = (NSMutableData *)[NSURLConnection sendSynchronousRequest:request returningResponse:nil error:nil];
    //    [request release];
    //    if (data) {
    //        [self downLoadFinish:data];
    //    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response  {
    if (arrayCount>0) {
        for (int i = 0; i<arrayCount; i++) {
            ...
            NSInteger changeID = [[MyDataBase sharedMyDataBase] getID];
            [[MyDataBase sharedMyDataBase] changeSaveMark:0 groupid:changeID];
            [[MyDataBase sharedMyDataBase] removeData];
        }
        arrayCount = 0;
    }
    else{
        ...
        NSInteger changeId=[[MyDataBase sharedMyDataBase] getGroupID];
        [[MyDataBase sharedMyDataBase] changeSaveMark:0 groupid:changeId];
        [[MyDataBase sharedMyDataBase] removeData];
...
//+ (void)setLocationSwitch:(BOOL)value{
//    BOOL isOpen = value;
//    NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
//    [defaults setBool:isOpen forKey:@"locationSwitch"];
...
-(NSString*)currentStatus{
    BOOL twoGisAvailable = [Reachabilitysdk isNetWorkVia2G];
    BOOL threeGisAvailable = [Reachabilitysdk isNetWorkVia3G];
    BOOL wifiIsAviable = [Reachabilitysdk isNetWorkViaWiFi];
